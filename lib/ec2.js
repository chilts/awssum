...
// addCommonArgs <- use the parent one
// strToSign     <- use the parent one
// signature     <- use the parent one
// addSignature  <- use the parent one
...
Ec2.prototype.describeInstances = function(args, callback) {
    ...
    self.addParam( params, 'Action', 'DescribeInstances' );
    self.performRequest({
        params : params,
    }, callback);
};
