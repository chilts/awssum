...
function decodeWwwFormUrlEncoded(body) {
    var form = {};
    body.split('&').forEach(function(v, i) {
        console.log(i, v);
        var keyValueArray = v.split('=');
        form[keyValueArray[0]] = keyValueArray[1];
    });
    return form;
}
...
AwsSum.prototype.send = function(operation, args, callback) {
    ...
    if ( debug ) {
        console.log('awssum.js:send(): options = ', options);
    }
...
AwsSum.prototype.request = function(options, callback) {
    ...
    // do the request
    var requestFn = self.protocol() === 'https' ? https.request : http.request;
