...
AwsSum.prototype.send = function(operation, args, callback) {
    ...
    // ---
    // See if we need a custom StreamHandler for async download
    if (args.StreamHandler !== undefined) {
        options.StreamHandler = args.StreamHandler;
    }
...
// standard stream function when getting API result
// all data returned is concatenated into a single string buffer available in res.body
function memory_stream(res, callback) {
...
AwsSum.prototype.request = function(options, callback) {
    var self = this;
    var streamHandler = (options.StreamHandler !== undefined) ? options.StreamHandler : memory_stream;
