...
AwsSum.prototype.send = function(operation, args, callback) {
    ...
    // if this isn't a number or array, it's an error
    if ( ! (_.isNumber(statusCode) || _.isArray(statusCode)) ) {
        // since this is a program error, we're gonna throw this one
    ...
    self.request( options, function(err, res) {
        ...
        if ( _.isArray(statusCode) ) {
            var statusCodeOk = false;
            statusCode.forEach(function(v) {
                if ( v === res.statusCode ) {
                    statusCodeOk = true;
                }
            });
        ...
        else if ( _.isNumber(statusCode) && statusCode !== res.statusCode ) {
            result.Code = 'AwsSum-IncorrectStatusCode';
            result.Message = 'Status code was not expected';
            // this was an error
            callback(result, null);
