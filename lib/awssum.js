...
// * Item.prefix.Y.Key1
// * Item.prefix.Y.Key2
function addParamArrayOfObjects(params, setName, array, prefix) {
    ...
    _.each(array, function(obj, i) {
        ...
        _.each(obj, function(value, key) {
            params.push({ 'name' : setName + prefix + '.' + (i+1) + '.' + key, 'value' : '' + value });
        });
    });
}
...
// * Item.prefix.2.Key2.prefix.2.Whatever
function addParamData(params, setName, data, prefix) {
    var self = this;
    var member = prefix ? '.' + prefix : '';

    if ( _.isString(data) || _.isNumber(data) || _.isBoolean(data) ) {
        params.push({ 'name' : setName, 'value' : '' + data });
        return;
    }
    ...
    if ( _.isArray(data) ) {
        ...
        _.each(data, function(value, i) {
            // if ( typeof value === 'string' ) {
            //     params.push({ 'name' : setName + member + '.' + (i+1), 'value' : value });
            //     return;
            // }

            // should be another data structure, so recurse down it
            self.addParamData(params, setName + member + '.' + (i+1), value, prefix);
        });
        return;
    }
    ...
    if ( _.isObject(data) ) {
        ...
        _.each(data, function(value, key) {
            // if ( typeof 'value' === 'string' ) {
            //     params.push({ 'name' : setName + '.' + key, 'value' : value });
            //     return;
            // }

            // should be another data structure, so recurse down it
            self.addParamData(params, setName + '.' + key, value, prefix);
        });
        return;
    }
}
...
AwsSum.prototype.send = function(operation, args, callback) {
    ...
    for ( argName in operation.args ) {
        ...
        else if ( spec.type === 'form-array-of-whatever' ) {
            addParamArrayObject( options.params, spec.setName, args[argName], spec.prefix );
        }
...
AwsSum.prototype.request = function(options, callback) {
    ...
    // if we have a port, then put that onto the request (can make local testing earier with a local server)
    if ( options.port ) {
