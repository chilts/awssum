...
function addParamArray(params, name, value, extra) {
    value = value || [];

    var prefix = '' + name + '.' + (extra ? extra + '.' : '');
...
AwsSum.prototype.send = function(operation, args, callback) {
    ...
    if ( operation.defaults ) {
        _.extend(args, operation.defaults);
    }
    ...
    if ( operation.method ) {
        if ( typeof operation.method === 'string' ) {
            ...
            method = operation.method.apply(self, [ args ]);
        }
        else {
    ...
    // build the host
    var host = self.host(args);
    if ( operation.host ) {
        if ( typeof operation.host === 'function' ) {
            host = operation.host.apply(self, [ args ]);
        }
    ...
    for ( var argName in operation.args ) {
        ...
        if ( spec.type === 'param' ) {
            addParamIfDefined( params, name, args[argName] );
        }
        else if ( spec.type === 'param-array' ) {
            addParamArray( params, name, args[argName], spec.prefix );
        }
        else if ( spec.type === 'param-array-set' ) {
            addParamArraySet( params, spec.setName, name, args[argName] );
        }
        else if ( spec.type === 'param-2d-array' ) {
            addParam2dArray( params, spec.setName, name, args[argName] );
        }
        else if ( spec.type === 'param-2d-array-set' ) {
            addParam2dArraySet( params, spec.setName, spec.subsetName, name, args[argName] );
        }
        else if ( spec.type === 'header' ) {
            setHeaderIfDefined( headers, name, args[argName] );
        }
    ...
    if ( extractBody !== 'xml' && extractBody !== 'json' && !_.isFunction(extractBody) ) {
        // since this is a program error, we're gonna throw this one
