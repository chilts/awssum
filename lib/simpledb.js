...
SimpleDB.prototype.performRequest = function(action, params, callBack) {
    ...
    req.on('error', function(err) {
        callBack({
            Code : 'AwsSum',
            Message : 'There was an error in the formation of the request',
            OriginalError : err
        }, null);
    });
}

SimpleDB.prototype.dataToAttributes = function(data) {
    var arr = [];
    ...
    if ( Array.isArray(data) ) {
        _.each(data, function(attr, key) {
            ...
            // if we have a replace value
            if ( typeof attr.replace !== 'undefined' ) {
                // coerce to a proper boolean
                attr.replace = attr.replace ? true : false;
        ...
        // if this is an object, loop through it now
...
SimpleDB.prototype.putAttributes = function(domainName, itemName, data, callBack) {
    ...
    // make our params
    // console.log(data);
    var params = this.dataToAttributes(data);
