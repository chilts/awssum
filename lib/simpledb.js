...
SimpleDB.prototype.createDomain = function(options, callback) {
    ...
    // set up our params
    var params = [];
    params.push({ 'name' : 'DomainName', 'value' : options.domainName });

    this.performRequest('CreateDomain', '/', params, callback);
};

SimpleDB.prototype.deleteAttributes = function(options, callback) {
    // check that we have a domain name
    if ( ! options.domainName ) {
        callback({ Code : 'AwsSumCheck', Message : 'No domain name given' }, null);
        return;
    ...
    // make our params
    var params = this.dataToDeleteAttributes(options.data);

    // add our domain and item names
    params.push({ 'name' : 'DomainName', 'value' : options.domainName });
    params.push({ 'name' : 'ItemName',   'value' : options.itemName });

    this.performRequest('DeleteAttributes', '/', params, callback);
};

SimpleDB.prototype.deleteDomain = function(options, callback) {
    // check that we have a domain name
    if ( ! options.domainName ) {
        callback({ Code : 'AwsSumCheck', Message : 'No domain name given' }, null);
        return;
...
SimpleDB.prototype.domainMetadata = function(options, callback) {
    // check that we have a domain name
    if ( ! options.domainName ) {
        callback({ Code : 'AwsSumCheck', Message : 'No domain name given' }, null);
        return;
...
SimpleDB.prototype.getAttributes = function(options, callback) {
    // check that we have a domain name
    if ( ! options.domainName ) {
        callback({ Code : 'AwsSumCheck', Message : 'No domain name given' }, null);
        return;
    ...
    if ( typeof options.attr !== 'undefined' ) {
        params.push({ 'name' : 'AttributeName', 'value' : options.attr });
    }

    params.push({ 'name' : 'ConsistentRead', 'value' : '' + (options.consistentRead ? true : false) });
...
SimpleDB.prototype.putAttributes = function(options, callback) {
    // check that we have a domain name
    if ( ! options.domainName ) {
        callback({ Code : 'AwsSumCheck', Message : 'No domain name given' }, null);
        return;
    ...
    var params = this.dataToPutAttributes(options.data);
...
SimpleDB.prototype.select = function(options, callback) {
    // check that we have a domain name
    ...
    // make our params
    var params = [];
    params.push({ 'name' : 'SelectExpression', 'value' : options.selectExpression });

    // do ConsistentRead if we have one
    if ( typeof options.consistentRead !== 'undefined' ) {
        if ( typeof options.consistentRead !== 'boolean' ) {
            callback({ Code : 'AwsSumCheck', Message : 'No consistent read must be a boolean value' }, null);
            return;
        }
        params.push({ 'name' : 'ConsistentRead', 'value' : '' + options.consistentRead });
    }

    // do NextToken if we have one
    if ( typeof options.nextToken !== 'undefined' ) {
        params.push({ 'name' : 'NextToken', 'value' : '' + options.nextToken });
    }
