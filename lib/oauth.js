...
OAuth.prototype.addCommonOptions = function(options, args) {
    ...
    // make the signature
    var strToSign = escape(options.method.toUpperCase()) + '&' + escape(self.protocol() + '://' + options.host + options.path);
    var pairs = _(options.params)
        .chain()
        .sortBy(function(p) { return p.name })
        .map(function(v, i) { return '' + v.name + '=' + escape(v.value) })
        .join('&')
        .value()
    ;
    ...
    var signatureKey = escape(self.consumerSecret()) + '&';
    if ( self.tokenSecret() ) {
        signatureKey += escape(self.tokenSecret());
    }
    ...
    if ( self.parameterTransmission() === 'header' ) {
        options.headers['Authorization'] = 'OAuth ' + [
            'oauth_version="1.0"',
            'oauth_consumer_key="' + escape(self.consumerKey()) + '"',
