...
Amazon.prototype.method = function() {
    return 'GET';
};
...
Amazon.prototype.addCommonArgs = function(args) {
    var self = this;
    ...
    args.params.push({ 'name' : 'AWSAccessKeyId', 'value' : self.accessKeyId() });
    args.params.push({ 'name' : 'Version', 'value' : self.version() });
    args.params.push({ 'name' : 'Timestamp', 'value' : date });
    args.params.push({ 'name' : 'SignatureVersion', 'value' : signatureVersion });
    args.params.push({ 'name' : 'SignatureMethod', 'value' : signatureMethod });
}
...
Amazon.prototype.strToSign = function(args) {
...
Amazon.prototype.addSignature = function(args, signature) {
    args.params.push({ 'name' : 'Signature', 'value' : signature });
};
...
Amazon.prototype.performRequest = function(options, callback) {
    ...
    options.verb    = options.verb || self.method();
...
Amazon.prototype.performOldRequest = function(action, path, params, callback) {
    var self = this;

    // DEPRECATED: performOldRequest() - you should be using performRequest()

    // need to add 'Action' to the params, since performRequest() no longer does this
    self.addParam( params, 'Action', action );

    // call the new performRequest()
    self.performRequest({
        path   : path,
        params : params,
    }, callback);
