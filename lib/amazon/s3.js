...
S3.prototype.deleteObject = function(args, callback) {
    ...
    var headers = {};
    self.setHeaderIfDefined( headers, 'x-amz-mfa', args.Mfa );
...
S3.prototype.deleteMultipleObjects = function(args, callback) {
    ...
    // do all headers (optional)
    var headers = {};
    self.setHeader( headers, 'Content-MD5', md5 );
    self.setHeader( headers, 'Content-Length', xml.length );
    self.setHeaderIfDefined( headers, 'x-amz-mfa', args.Mfa );
...
S3.prototype.getObject = function(args, callback) {
    ...
    self.setHeaderIfDefined( headers, 'Range',               args.Range             );
    self.setHeaderIfDefined( headers, 'If-Modified-Since',   args.IfModifiedSince   );
    self.setHeaderIfDefined( headers, 'If-Unmodified-Since', args.IfUnmodifiedSince );
    self.setHeaderIfDefined( headers, 'If-Match',            args.IfMatch           );
    self.setHeaderIfDefined( headers, 'If-None-Match',       args.IfNoneMatch       );
...
S3.prototype.objectMetadata = function(args, callback) {
    ...
    var headers = {};
    self.setHeaderIfDefined( headers, 'Range',             args.Range             );
    self.setHeaderIfDefined( headers, 'IfModifiedSince',   args.IfModifiedSince   );
    self.setHeaderIfDefined( headers, 'IfUnmodifiedSince', args.IfUnmodifiedSince );
    self.setHeaderIfDefined( headers, 'IfMatch',           args.IfMatch           );
    self.setHeaderIfDefined( headers, 'IfNoneMatch',       args.IfNoneMatch       );
...
S3.prototype.putObject = function(args, callback) {
    ...
    var headers = {};
    self.setHeader(headers, 'Content-Length', args.ContentLength);

    // now the optional ones
    self.setHeaderIfDefined( headers, 'Cache-Control',                args.CacheControl         );
    self.setHeaderIfDefined( headers, 'Content-Disposition',          args.ContentDisposition   );
    self.setHeaderIfDefined( headers, 'Content-Encoding',             args.ContentEncoding      );
    self.setHeaderIfDefined( headers, 'Content-MD5',                  args.ContentMd5           );
    self.setHeaderIfDefined( headers, 'Content-Type',                 args.ContentType          );
    self.setHeaderIfDefined( headers, 'Expect',                       args.Expect               );
    self.setHeaderIfDefined( headers, 'Expires',                      args.Expires              );
    self.setHeaderIfDefined( headers, 'x-amz-acl',                    args.Acl                  );
    self.setHeaderIfDefined( headers, 'x-amz-meta',                   args.Meta                 );
    self.setHeaderIfDefined( headers, 'x-amz-server-side-encryption', args.ServerSideEncryption );
    self.setHeaderIfDefined( headers, 'x-amz-storage-class',          args.StorageClass         );
...
S3.prototype.copyObject = function(args, callback) {
    ...
    // firstly, do the headers
    var headers = {};
    self.setHeader(headers, 'x-amz-copy-source', '/' + args.SourceBucket + '/' + args.SourceObject);

    // now the optional ones
    self.setHeaderIfDefined( headers, 'x-amz-acl',                             args.Acl                         );
    self.setHeaderIfDefined( headers, 'x-amz-metadata-directive',              args.MetadataDirective           );
    self.setHeaderIfDefined( headers, 'x-amz-copy-source-if-match',            args.CopySourceIfMatch           );
    self.setHeaderIfDefined( headers, 'x-amz-copy-source-if-none-match',       args.CopySourceIfNoneMatch       );
    self.setHeaderIfDefined( headers, 'x-amz-copy-source-if-unmodified-since', args.CopySourceIfUnmodifiedSince );
    self.setHeaderIfDefined( headers, 'x-amz-copy-source-if-modified-since',   args.CopySourceIfModifiedSince   );
    self.setHeaderIfDefined( headers, 'x-amz-server-side-encryption',          args.ServerSideEncryption        );
    self.setHeaderIfDefined( headers, 'x-amz-storage-class',                   args.StorageClass                );
    ...
    self.setHeader( headers, 'Content-Length', 0 );
