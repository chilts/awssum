...
S3.prototype.deleteObject = function(args, callback) {
    ...
    var headers = {};
    self.addHeaderIfDefined( headers, 'x-amz-mfa', args.Mfa );
...
S3.prototype.deleteMultipleObjects = function(args, callback) {
    ...
    // do all headers (optional)
    var headers = {};
    self.addHeader( headers, 'Content-MD5', md5 );
    self.addHeader( headers, 'Content-Length', xml.length );
    self.addHeaderIfDefined( headers, 'x-amz-mfa', args.Mfa );
...
S3.prototype.getObject = function(args, callback) {
    ...
    self.addHeaderIfDefined( headers, 'Range',               args.Range             );
    self.addHeaderIfDefined( headers, 'If-Modified-Since',   args.IfModifiedSince   );
    self.addHeaderIfDefined( headers, 'If-Unmodified-Since', args.IfUnmodifiedSince );
    self.addHeaderIfDefined( headers, 'If-Match',            args.IfMatch           );
    self.addHeaderIfDefined( headers, 'If-None-Match',       args.IfNoneMatch       );
...
S3.prototype.objectMetadata = function(args, callback) {
    ...
    var headers = {};
    self.addHeaderIfDefined( headers, 'Range',             args.Range             );
    self.addHeaderIfDefined( headers, 'IfModifiedSince',   args.IfModifiedSince   );
    self.addHeaderIfDefined( headers, 'IfUnmodifiedSince', args.IfUnmodifiedSince );
    self.addHeaderIfDefined( headers, 'IfMatch',           args.IfMatch           );
    self.addHeaderIfDefined( headers, 'IfNoneMatch',       args.IfNoneMatch       );
...
S3.prototype.putObject = function(args, callback) {
    ...
    var headers = {};
    self.addHeader(headers, 'Content-Length', args.ContentLength);

    // now the optional ones
    self.addHeaderIfDefined( headers, 'Cache-Control',                args.CacheControl         );
    self.addHeaderIfDefined( headers, 'Content-Disposition',          args.ContentDisposition   );
    self.addHeaderIfDefined( headers, 'Content-Encoding',             args.ContentEncoding      );
    self.addHeaderIfDefined( headers, 'Content-MD5',                  args.ContentMd5           );
    self.addHeaderIfDefined( headers, 'Content-Type',                 args.ContentType          );
    self.addHeaderIfDefined( headers, 'Expect',                       args.Expect               );
    self.addHeaderIfDefined( headers, 'Expires',                      args.Expires              );
    self.addHeaderIfDefined( headers, 'x-amz-acl',                    args.Acl                  );
    self.addHeaderIfDefined( headers, 'x-amz-meta',                   args.Meta                 );
    self.addHeaderIfDefined( headers, 'x-amz-server-side-encryption', args.ServerSideEncryption );
    self.addHeaderIfDefined( headers, 'x-amz-storage-class',          args.StorageClass         );
...
S3.prototype.copyObject = function(args, callback) {
    ...
    // firstly, do the headers
    var headers = {};
    self.addHeader(headers, 'x-amz-copy-source', '/' + args.SourceBucket + '/' + args.SourceObject);

    // now the optional ones
    self.addHeaderIfDefined( headers, 'x-amz-acl',                             args.Acl                         );
    self.addHeaderIfDefined( headers, 'x-amz-metadata-directive',              args.MetadataDirective           );
    self.addHeaderIfDefined( headers, 'x-amz-copy-source-if-match',            args.CopySourceIfMatch           );
    self.addHeaderIfDefined( headers, 'x-amz-copy-source-if-none-match',       args.CopySourceIfNoneMatch       );
    self.addHeaderIfDefined( headers, 'x-amz-copy-source-if-unmodified-since', args.CopySourceIfUnmodifiedSince );
    self.addHeaderIfDefined( headers, 'x-amz-copy-source-if-modified-since',   args.CopySourceIfModifiedSince   );
    self.addHeaderIfDefined( headers, 'x-amz-server-side-encryption',          args.ServerSideEncryption        );
    self.addHeaderIfDefined( headers, 'x-amz-storage-class',                   args.StorageClass                );
