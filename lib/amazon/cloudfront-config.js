...
function pathDistributionIdConfig(args) {
    return '/' + this.version() + '/distribution/' + args.DistributionId + '/config';
}

function pathDistributionInvalidation(args) {
    return '/' + this.version() + '/distribution/' + args.DistributionId + '/invalidation';
}

function pathDistributionInvalidationId(args) {
...
function bodyStreamingDistributionConfig(args) {
    // create the XML
    var data = {
        _attr : { 'xmlns' : 'http://cloudfront.amazonaws.com/doc/2010-11-01/' },
    };
