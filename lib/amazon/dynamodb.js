...
var DynamoDB = function(accessKeyId, secretAccessKey, awsAccountId, region) {
    var self = this;

    // call the superclass for initialisation
    DynamoDB.super_.call(this, accessKeyId, secretAccessKey, awsAccountId, region);
    ...
    self._sts = new stsService(accessKeyId, secretAccessKey, awsAccountId, amazon.US_EAST_1);
...
DynamoDB.prototype.strToSign = function(options, args) {
    ...
    // create the string we need to sign
    var strToSign = '';

    if ( false ) {
        strToSign += options.method + "\n";
        strToSign += "/\n";
        strToSign += "\n"; // querystring, but DynamoDB doesn't use them
        strToSign += canonicalizedHeaders + "\n";
        strToSign += options.body;

        console.log('===============================================================================');
        console.log('1) StrToSign :', strToSign + '(Ends)');
        console.log('===============================================================================');

        var strToSign = [options.method, '/', '', canonicalHeaders, options.body].join("\n");

        console.log('===============================================================================');
        console.log('2) StrToSign :', strToSign + '(Ends)');
        console.log('===============================================================================');
...
_.each(operations, function(operation, operationName) {
    ...
    var doRequest = function(args, callback) {
        ...
        mainOperation.apply(self, [ args, function(err, data) {
            if ( err ) {
                // console.log('-------------------------------------------------------------------------------');
                // console.log('Error with the mainOperation');
                // console.log(err);
                // console.log('-------------------------------------------------------------------------------');
                return;

                if ( err.Something.Something.Something === 'Something' ) {
    ...
    var wrapper = function(args, callback) {
        ...
        // if we have a session token already, just do the request
        if ( !_.isUndefined(self._sessionToken) ) {
            // console.log('wrapper(): we already have a session token, just do the request');
        ...
        self._sts.GetSessionToken(function(err, data) {
            ...
            // console.log('GetSessionToken(): got a session token');
